...
def test_get_v_i():
    P = Polygon([Vertex("V1", 5.6, 2.9), Vertex(
...
def test_get_rotation_angle():
    P = Polygon([Vertex("V1", 1.3, 2.1), Vertex("V2", 2.7, 3.9), Vertex(
        "V3", 4.9, 1.5), Vertex("V4", 0.5, 0.8), Vertex("V5", 3.5, 1.8)])
...
def test_get_winding_number():
    ...
    P = Polygon([Vertex("V1", -1.4, -1.2), Vertex("V2", 1.7, 1.6),
                Vertex("V3", 1.3, -1.4), Vertex("V4", -1.5, 1.9)])
    winding_number = P.get_winding_number()
    expected_sum = (P.get_rotation_angle(1) + P.get_rotation_angle(2) +
                    P.get_rotation_angle(3) + P.get_rotation_angle(4))
...
def test_center_polygon():
    P = Polygon([Vertex("V1", 1.5, 2.4), Vertex(
...
def test_is_clockwise():
    P_1 = Polygon([Vertex("V1", -1, 0), Vertex("V2", math.sqrt(3) /
                  2, 0.5), Vertex("V3", 1, 0), Vertex("V4", 0, -1)])
    assert P_1.is_clockwise(P_1.vertices)
    P_2 = Polygon([Vertex("V1", 0.5, math.sqrt(3) / 2),
